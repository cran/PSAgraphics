box.psa<-function(continuous, treatment = NULL, strata = NULL, 
 ...
 legend.xy=NULL, legend.labels = 
 c("Control", "Treatment", "C/T Means Compared", "KS p-values", "Strata-Treatment Size"),
 pts=TRUE, balance = FALSE, B=1000, ...){
...
cts<-cbind(continuous,treatment,strata)
cs.0<-subset(cts,treatment==0,select=c(continuous,strata))
cs.1<-subset(cts,treatment==1,select=c(continuous,strata))  
size<-table(treatment,strata)
                    
if(balance){bal.ms<-bal.ms.psa(continuous,treatment,strata,B)
...
boxplot(continuous ~ strata,
        ...
        subset= treatment == "0", col=col[1],
        xlab=xlab, ...)

boxplot(continuous ~ strata,  add = TRUE,axes=FALSE,
        boxwex = boxwex, at = 1:s.d + offset,
        subset= treatment == "1", col=col[2])

#Add point clouds to boxplots
if(pts){
  for(i in 1:s.d){cs.0.strat<-subset(cs.0,cs.0[,2]==i)
                  points(jitter(cs.0.strat[,2]-offset,amount=.25*boxwex),
                  jitter(cs.0.strat[,1],amount=.10*boxwex),col="yellow3",cex=.8)}
  for(i in 1:s.d){cs.1.strat<-subset(cs.1,cs.1[,2]==i)
                  points(jitter(cs.1.strat[,2],amount=.25*boxwex)+offset,
...
last<-length(x.t)
x.coord<-x.t[-last]
y.t<-aggregate(cts,by=list(treatment=treatment,strata=strata),FUN=mean)
y.tt<-NULL
...
if(balance){legend(x=legend.xy[1],y=legend.xy[2], legend = legend.labels,
           ...
           lwd=c(2,2,2,2),cex=.8)}
           else{legend(x=legend.xy[1],y=legend.xy[2], legend = legend.labels[c(1,2,3,5)],
